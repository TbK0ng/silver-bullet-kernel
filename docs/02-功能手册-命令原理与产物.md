...
- 关键检查（摘核心）：
  ...
  - 要求本次变更同时包含配置中要求的文档文件（默认含 `docs/02`、`docs/05`、`docs/06`）。
- 产物：
...
- 常见示例：
  ...
  - `npm run sbk -- policy`
  - `npm run sbk -- install --target-repo-root E:\repo --preset minimal`
  - `npm run sbk -- upgrade --target-repo-root E:\repo --preset full`
  - `npm run sbk -- greenfield --adapter node-ts --project-name demo-app --target-repo-root E:\repo`
  - `npm run sbk -- flow run --target-repo-root E:\repo --decision-mode auto`
...
- 作用：一条命令串联 install/greenfield(or brownfield)/blueprint/intake/adapter doctor。
