...
- `sbk flow run [--target-repo-root <path>] [--decision-mode auto|ask] [--scenario auto|greenfield|brownfield] [--adapter <name>] [--profile strict|balanced|lite] [--project-name <name>] [--project-type backend|frontend|fullstack] [--blueprint <name|none>] [--channel stable|beta] [--preset minimal|full] [--with-install] [--skip-verify] [--fleet-roots <path1,path2,...>]`
...
- 可联动执行：install -> greenfield/blueprint -> intake -> adapter doctor -> optional verify/fleet
