...
| 一键全流程 | 自动串联 install/blueprint/intake/doctor | `npm run sbk -- flow run --target-repo-root <path> --decision-mode auto` |
...
| `npm run sbk -- flow run ...` | `<target>/.metrics/flow-run-report.json`, `<target>/.metrics/intake-readiness.json`, `<target>/.metrics/blueprint-verify.json` |
...
一句话总结：  
日常看 `docs` 和 `docs`，执行看命令，追责看 `.metrics` 与 OpenSpec 产物。
