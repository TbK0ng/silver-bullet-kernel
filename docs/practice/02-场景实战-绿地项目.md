...
1) sbk greenfield --adapter node-ts --project-name demo-app --project-type backend --target-repo-root .
2) sbk blueprint list，并选择一个 stable 蓝图（优先 api-service）执行 apply + verify
3) sbk flow run --scenario greenfield --decision-mode auto --adapter node-ts --project-type backend --channel stable --target-repo-root .
4) sbk verify:fast
