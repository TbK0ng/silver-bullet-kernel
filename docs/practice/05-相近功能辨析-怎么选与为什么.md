...
- `flow run`：按参数与上下文执行条件化阶段并产出执行报告。
...
- 你已经熟悉参数，且希望一次跑完主要流程。
- 你明确知道 install/verify/fleet 是否应触发（`--with-install` / `--skip-verify` / `--fleet-roots`）。
...
- `new-change`：创建变更容器。
- `explore`：显式进入探索/复盘上下文。
- `migrate-specs`：把 change 下的 delta 迁移到 canonical specs。

### 常见正确顺序

1. `sbk new-change <id>`
2. `sbk explore --change <id>`（显式探索/澄清）
3. 执行开发与文档变更
