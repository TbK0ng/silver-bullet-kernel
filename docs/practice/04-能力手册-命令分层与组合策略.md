...
- install 仅在 `--with-install` 时触发。
- `--channel beta` 会自动放开 beta 资产；也可显式使用 `--allow-beta`。
- `--force` 会放开覆盖行为；blueprint 在 `--with-install` 或已有 CI 工作流时也可能进入 force 模式。
- verify-fast 默认触发，`--skip-verify` 时跳过。
- fleet 仅在 `--fleet-roots` 时触发。
- `flow run` 不会隐式触发 `new-change`、`explore`、`record-session`。
...
- `flow run --channel beta` 会自动放开 beta 资产；在 stable 通道下使用 beta 资产时可显式传 `--allow-beta`。
- 覆盖行为优先显式 `--force`；但在 `flow run` 内，`--with-install` 或已存在 CI 工作流也可能让 blueprint 进入 force 模式。
...
```powershell
sbk fleet collect --roots <repo-a>,<repo-b>
...
- `sbk new-change` 仅创建变更容器。
- `sbk explore` 需要显式调用，不是 `flow run` 的隐式阶段。
- `sbk record-session` 也是显式会话动作。
...
- 明确写出“显式步骤列表”让 AI 代执行（不要假设命令会隐式补齐探索步骤）。
