...
- generated_at_utc: 2026-02-13 15:52:50Z
- outcome: PASS
...
| --- | --- | --- | --- | --- | --- |
| Metrics source path resolved | warn | PASS | E:\docc\trellis-worktrees\sbk-codex-strict-plan13-philosophy\.metrics\verify-runs-ci.jsonl | recommended: non-empty path | Run metrics collection with deterministic telemetry path configuration. |
...
| Lead time P90 within threshold | warn | PASS | 12.45 h | <= 72 h | Split oversized changes and archive in smaller batches. |
...
| Token cost status available | warn | PASS | unavailable | recommended: available | Run npm run metrics:token-cost -- -Source <provider> -TotalCostUsd <amount> to publish token-cost summary. |
