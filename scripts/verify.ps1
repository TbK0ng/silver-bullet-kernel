...
$run = New-VerifyRun -Mode "full" -RepoRoot $sbkRoot
...
function Invoke-Step {
  ...
  try {
    ...
    throw "[verify] step failed: $Name. $($_.Exception.Message)"
  }
}
...
try {
  ...
  foreach ($command in $commands) {
    Invoke-Step `
      -Name "adapter[$($runtime.adapter)] full: $command" `
      -Command $command `
      -Action {
  ...
  Write-Host "[verify] completed adapter=$($runtime.adapter) profile=$($runtime.profile)"
} catch {
  $failedStep = if ($run.steps.Count -gt 0) { $run.steps[-1].name } else { "verify bootstrap" }
